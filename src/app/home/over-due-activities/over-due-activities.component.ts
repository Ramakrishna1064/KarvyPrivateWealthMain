import { DashBoardChildObject } from './../../shared/objectMappers/DahBoardChildObject';
...
@Component({
  selector: 'app-over-due-activities',
  templateUrl: './over-due-activities.component.html',
  styleUrls: ['./over-due-activities.component.scss']
})
export class OverDueActivitiesComponent implements OnInit {
   ...
   */
  ngOnInit() {
    this.getOverDueActivitiesList(0);
  }
   ...
   * getLeadsList
   */
  getOverDueActivitiesList(pageNumber: number) {
    ...
    if (navigator.onLine) {
      ...
      this.dashBoardService.getOverDueActivititesList(data).subscribe(leadsList => {
        console.log("over due final data---->" + JSON.stringify(leadsList));
        const leadsData: DashBoardParentObject = leadsList;
        this.length = leadsData.totalEntities != null ? leadsData.totalEntities : 0;
        this.dataSource = new MatTableDataSource(leadsData.opsCallMeetingResponse != null ?
          leadsData.opsCallMeetingResponse : Array(0));
  ...
  public getServerData(event?: PageEvent) {
    this.getOverDueActivitiesList(event.pageIndex);
  }
